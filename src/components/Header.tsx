import React, { useState, useRef, useEffect } from "react";
import { Menu } from "lucide-react";
import { useLogoutMutation } from "@/redux/features/auth/authApi";
import { useNavigate } from "react-router-dom";
import { useGetAllNbfcQuery } from "@/redux/features/nbfc/nbfcApi";
import { useDispatch } from "react-redux";
import { setSelectedNbfc } from "@/redux/features/nbfc/nbfcSlice";
import {
  Select,
  SelectTrigger,
  SelectValue,
  SelectContent,
  SelectItem,
} from "@/components/ui/select";

const Header: React.FC = () => {
  const [open, setOpen] = useState(false);
  const [selectedNbfc, setSelectedNbfcLocal] = useState("all");
  const menuRef = useRef<HTMLDivElement>(null);

  const dispatch = useDispatch();
  const [logoutTrigger] = useLogoutMutation();
  const navigate = useNavigate();

  /** ===========================
   * Fetch NBFC LIST
   * =========================== */
  const {
    data: nbfcData,
    isLoading: nbfcLoading,
    isFetching: nbfcFetching,
    error: nbfcError,
  } = useGetAllNbfcQuery(undefined, { refetchOnMountOrArgChange: true });

  /** Normalize NBFC array */
  const nbfcArray =
    Array.isArray(nbfcData?.data?.items)
      ? nbfcData.data.items
      : Array.isArray(nbfcData?.data)
        ? nbfcData.data
        : Array.isArray(nbfcData)
          ? nbfcData
          : [];

  const handleLogout = async () => {
    try {
      await logoutTrigger();
    } catch (err) {
      console.error("Logout failed", err);
    }
  };

  // Close menu on outside click
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (menuRef.current && !menuRef.current.contains(e.target as Node)) {
        setOpen(false);
      }
    }
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, []);

  // Auto-select first NBFC after data loads
  useEffect(() => {
    if (nbfcArray.length > 0 && selectedNbfc === "all") {
      const first = nbfcArray[0];
      const partnerId = String(first.partner_id ?? first.id);
      setSelectedNbfcLocal(partnerId);
      dispatch(setSelectedNbfc(first)); // ✅ set in Redux
    }
  }, [nbfcArray]);

  return (
    <header className="sticky top-0 z-50 bg-white shadow-[0_2px_0_rgba(0,0,0,0.05)]">
      <div className="w-full">
        <nav className="flex items-center justify-between py-3">

          {/* Left Navigation */}
          <div>
            <div className="px-3 mb-2">
              <Select
                value={selectedNbfc}
                onValueChange={(value) => {
                  setSelectedNbfcLocal(value); // local state for UI
                  const nbfc = nbfcArray.find(
                    (nb) => String(nb.partner_id ?? nb.id) === value
                  ) ?? null;
                  dispatch(setSelectedNbfc(nbfc)); // ✅ set Redux state
                }}
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Select NBFC" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All NBFCs</SelectItem>
                  {nbfcArray.map((nbfc: any) => (
                    <SelectItem
                      key={nbfc.partner_id ?? nbfc.id}
                      value={String(nbfc.partner_id ?? nbfc.id)}
                    >
                      {nbfc.nbfc_name ?? nbfc.name ?? "-"}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          </div>

          {/* RIGHT MENU */}
          <div className="relative mr-6" ref={menuRef}>
            <button
              onClick={() => setOpen(!open)}
              className="flex items-center space-x-2 focus:outline-none"
            >
              <Menu className="w-6 h-6 text-gray-600 cursor-pointer" />
            </button>

            {open && (
              <div className="absolute right-0 mt-3 w-64 bg-white rounded-lg shadow-lg border border-gray-100 py-3 animate-fade-in">
                <hr />

                {/* Regular menu items */}
                <a
                  href="#"
                  className="block px-4 py-2 text-gray-700 hover:bg-gray-100"
                >
                  Profile
                </a>
                <a
                  href="#"
                  className="block px-4 py-2 text-gray-700 hover:bg-gray-100"
                >
                  Settings
                </a>
                <button
                  className="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100"
                  onClick={handleLogout}
                >
                  Logout
                </button>
              </div>
            )}
          </div>
        </nav>
      </div>
    </header>
  );
};

export default Header;
